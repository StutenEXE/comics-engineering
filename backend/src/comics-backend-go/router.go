...
func startRouter() {
	...
	{
		// Serie services
		protected.POST("/contribute", services.CreateUserSubmission)
	}
	...
	admProtected.Use(middleware.AdminSessionAuth())
	{
		...
		// Admin user submission services
		admProtected.POST("/user-submissions/approve", services.ApproveUserSubmission)
		admProtected.GET("/user-submissions/list", services.GetUserSubmissionsByUserID)
	}
