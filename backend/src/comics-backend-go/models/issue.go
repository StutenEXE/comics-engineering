...
type Issue struct {
	...
	IssueSerie   *IssueSerieModel `json:"issueSerie"`
...
func linkIssueToBook(is *Issue, bk *BookModel) error {
	query := "INSERT INTO books_issues (book_id, issue_id) VALUES ($1, $2)"
	_, err := database.PgDb.Exec(query, bk.ID, is.ID)
	if err != nil {
		return err
	}
	return nil
