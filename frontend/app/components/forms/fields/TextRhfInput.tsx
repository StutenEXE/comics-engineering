...
export const TextRhfInput = forwardRef<TextRhfInputHandle, TextRhfInputProps>(
  function TextRhfInput(
    { label, registration, inputProps, error, tooltip, className },
    ref,
  ) {
    ...
    const mergedRef = (element: HTMLInputElement | null) => {
      ...
      if (typeof registerRef === "function") {
        ...
        (registerRef as React.RefObject<HTMLInputElement | null>).current =
          element;
      }
    };
    ...
    return (
      <div className={twMerge("flex flex-col gap-1.5", className)}>
        <label className="w-full flex items-center gap-2 text-xs font-medium uppercase tracking-widest text-white/40">
        ...
        <Input
          ...
          {...restRegistration}
          {...inputProps}
          className="w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-indigo-500/70 focus:ring-1 focus:ring-indigo-500/30 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
        />
