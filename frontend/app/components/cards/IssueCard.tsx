...
export function IssueCard({ issue, className }: IssueCardProps) {
  ...
  return (
    <Link to={`/issue/${issue.id}`} className={twMerge("group block w-100", className)}>
      <div className="flex items-center justify-between gap-4 px-0 py-2 rounded-md border border-white/8 bg-white/3 hover:border-indigo-500/30 hover:bg-white/5 transition-all">
